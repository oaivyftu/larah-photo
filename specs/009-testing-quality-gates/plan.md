...
| V. Critical User Flows Require Test Coverage (aspirational) | This feature is the framework adoption the principle calls for. **Follow-up required**: run `/speckit-constitution` after implementation to formally name Vitest as the adopted framework and retire the manual-test-plan fallback (MINOR bump per Governance's versioning policy — expanded/resolved guidance, not a removal) | ⚠️ Pass with required follow-up (tracked, not a blocking violation — the principle is explicitly aspirational until this adoption happens) |
...
*Post-Phase 1 re-check*: Design artifacts (data-model.md, contracts/npm-scripts.md, quickstart.md) introduce no additional dependencies or principle exposure beyond what's listed above. Gate still passes.
