...
**Finding**: The same Next.js docs state plainly: _"Since `async` Server Components are new to the React ecosystem, Vitest currently does not support them... we recommend using E2E tests for `async` components."_
...
**What Vitest _can_ cover** (the actual surface for this feature's unit + mock tests):
...
**Finding**: spec FR-003 originally required the commit to be _blocked_ on a formatting failure, while data-model.md specified `prettier --write` in lint-staged. Those cannot both hold: `--write` rewrites the file and exits 0, so it never blocks.
