...
**One tension worth naming**: Principle VII says a raw hex or bare `font-size` "is a violation, not a shortcut", which reads as zero-tolerance. This feature deliberately permits single-use literals that carry an explanatory comment. That is not a softening — it is what makes the rule enforceable at this codebase's actual starting point, where 238 declarations bypass the token layer and two thirds of the type sizes are fluid ramps a fixed scale cannot express (research.md §1–§2). Minting ~40 single-use tokens would satisfy the letter of VII while making the design system less useful. If experience shows the exception being abused, tighten it by amendment rather than by informal drift.
